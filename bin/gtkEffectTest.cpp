...
/*
	This is a small example of using libvcvideo with Gtkmm and our effects too.
...
class EffectPanel : public Gtk::Frame {
	public:
		EffectPanel();
		~EffectPanel();
};
...
class Viewer : public Gtk::Window {
	public:
		Viewer(Glib::ustring deviceName) : vd(deviceName), running(true) {
			...
			set_title("libvcvideo - Gtk Effect Test - "+deviceName);
	...
	private:
		...
		bool init () {
			...
			for(std::vector<std::string>::iterator it = effectsByName.begin(); it != effectsByName.end(); ++it)
				effectsCombo.append_text(*it);
			vbox.pack_start(effectsCombo,false,false,0);
			effectsCombo.signal_changed().connect(sigc::mem_fun(*this,&Viewer::effectChange));
		...
		void effectChange () {
			// Change active effect name
			// Change effect panel
			// Generate effectArguments?
		}
